...
##########################################################################
## Residual Depth-Image Cross-Attention Block (RDICAB)
class RDICAB(nn.Module):
    def __init__(self, dim, num_heads=4, ffn_expansion_factor=2, bias=False, LayerNorm_type='WithBias',scale=1.0):
        super(RDICAB, self).__init__()

        self.norm1_image = LayerNorm(dim, LayerNorm_type)
        self.norm1_depth = LayerNorm(dim, LayerNorm_type)
    ...
    def forward(self, image, depth):
        ...
        # depth: b, c, h, w
        # return: b, c, h, w
        assert image.shape == depth.shape, 'the shape of image doesnt equal to event'
        b, c , h, w = image.shape
        fused = image + self.attn(self.norm1_image(image), self.norm1_depth(depth)) # b, c, h, w
